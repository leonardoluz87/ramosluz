...
def adicionar_item():
    ...
    if descricao or marca or quantidade or preco:  # Permite adicionar mesmo que alguns campos estejam vazios
        if descricao and marca and quantidade and preco:
            itens.append((descricao, marca, int(quantidade), float(preco)))
            itens_list.insert(tk.END, f"{descricao} - {marca} - QTD: {quantidade} - R$ {preco}")
            descricao_entry.delete(0, tk.END)
            marca_entry.delete(0, tk.END)
            quantidade_entry.delete(0, tk.END)
            preco_entry.delete(0, tk.END)
        else:
            messagebox.showinfo("Informação", "Item incompleto, mas será adicionado.")
            itens.append((descricao, marca, quantidade, preco))  # Adiciona mesmo assim
            itens_list.insert(tk.END, f"{descricao} - {marca} - QTD: {quantidade} - R$ {preco}")
            descricao_entry.delete(0, tk.END)
            marca_entry.delete(0, tk.END)
            quantidade_entry.delete(0, tk.END)
...
# Lista de itens
itens_list = tk.Listbox(janela, height=5, width=50)
